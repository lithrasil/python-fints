...
from fints.utils import decode_phototan_image, mt940_to_array
...
def _mt940_single_statement_bundle(data):
    bundles = [b for b in mt940_to_array(data) if b.transactions]
    assert len(bundles) == 1
    return bundles[0]


def test_mt940_purpose_newline_between_subfields():
    """Structured :86: ?20/?21 lines become \\n-separated purpose (not SWIFT wrap)."""
    data = (
        "\r\n-\r\n"
        ":20:STARTUMS\r\n"
        ...
        ":86:051?00TXT?20LINE1?21LINE2\r\n"
        ":62F:C150101EUR1223,57\r\n"
        "-\r\n"
    )
    bundle = _mt940_single_statement_bundle(data)
    assert bundle.transactions[0].data["purpose"] == "LINE1\nLINE2"


def test_mt940_physical_wrap_inside_86_is_stripped_before_purpose():
    """Physical newlines inside :86: join text; semantic breaks stay at ?Nn only."""
    data = (
        "\r\n-\r\n"
        ":20:STARTUMS\r\n"
        ":25:12345678/0000000001\r\n"
        ...
        ":60F:C150101EUR1041,23\r\n"
        ":61:150101C182,34NMSCNONREF\r\n"
        ":86:051?00TXT?20ABCDE\r\n"
        "FG?21NEXT\r\n"
        ":62F:C150101EUR1223,57\r\n"
        "-\r\n"
    )
    bundle = _mt940_single_statement_bundle(data)
    assert bundle.transactions[0].data["purpose"] == "ABCDEFG\nNEXT"
